...
public class PeopleManager {
                ...
                if (!forename.isEmpty() && !name.isEmpty()) {
                    ...
                    if (p.isThereIncoherence()) {
                        ...
                        if (PeopleManager.alwaysCheckCSVInputs) {
                            ...
                            System.out.println();
                            // Do not close inputScanner here, as it would close System.in for the rest of the program
                        }
    ...
    public void serializeExchanges() {
        ...
        for (CountryVisit visit : this.countryVisits) {
            for (Exchange e : visit.getRawExchanges()) {
    ...
    public List<CountryVisit> getCountryVisits() {
        return this.countryVisits;
    }

    // Returns a list of country visit corresponding to the exchanges in the history (for graphical use)
    public List<CountryVisit> getCountryVisitsFromHistory() {
        // Map: key = year:guestCountry:hostCountry, value = CountryVisit
        Map<String, CountryVisit> visitMap = new HashMap<>();
        for (Exchange e : PeopleManager.history) {
            int year = e.getYear();
            String guestCountry = e.getGuest().getCriteriaValue(Criteria.COUNTRY_OF_ORIGIN);
            String hostCountry = e.getHost().getCriteriaValue(Criteria.COUNTRY_OF_ORIGIN);
            if (guestCountry == null || hostCountry == null) continue;

            String key = year + ":" + guestCountry.toLowerCase() + ":" + hostCountry.toLowerCase();
            CountryVisit visit = visitMap.get(key);
            if (visit == null) {
                visit = new CountryVisit(year, guestCountry, hostCountry);
                visitMap.put(key, visit);
            }
            // Ajoute l'échange à la visite (en utilisant setArbitraryBestExchanges plus tard)
            visit.getRawExchanges().add(e);
        }
        // Pour chaque visite, on met à jour les exchanges arbitraires
        for (CountryVisit visit : visitMap.values()) {
            visit.setArbitraryBestExchanges(visit.getRawExchanges());
        }
        return new ArrayList<>(visitMap.values());
    }

    public List<CountryVisit> getCountryVisitsFromBans() {
        // Map: key = year:guestCountry:hostCountry, value = CountryVisit
        Map<String, CountryVisit> visitMap = new HashMap<>();
        for (Exchange e : PeopleManager.bans) {
            int year = e.getYear();
            String guestCountry = e.getGuest().getCriteriaValue(Criteria.COUNTRY_OF_ORIGIN);
            String hostCountry = e.getHost().getCriteriaValue(Criteria.COUNTRY_OF_ORIGIN);
            if (guestCountry == null || hostCountry == null) continue;
    ...
    // APP
    public static List<Exchange> getExchangeOfCountryvisit(CountryVisit cv){

        List<Exchange> cvExchanges = new ArrayList<Exchange>();
        for(Exchange temp : PeopleManager.history){
            if(cv.getHostCountry().equals(temp.getHost().getCountry()) && cv.getGuestCountry().equals(temp.getGuest().getCountry()) && cv.getYear() == temp.getYear()){
                cvExchanges.add(temp);
            }
        }
        return cvExchanges;
    }
