...
public class Exchange implements Serializable {
    ...
    public double evaluateAffinity() { 
        ...
        // Age difference
        changeScore((int) Math.round(this.birthDateHandler() * 10), Criteria.BIRTH_DATE);
    ...
    */
    ...
    // Useful for graphical interface
    public String toGraphicalString() {
        return "Échange entre " + this.getHost().getFullName() + " et " + this.getGuest().getFullName() + " avec une affinité de " + this.affinityScore ;
    }

    // Displays the criteria that mattered in the exchange, whether its none or not met criteria
    public String criteriaThatMatteredToString() {
        Set<Criteria> notMet = this.getNotMetCriteria();
        if (notMet == null || notMet.isEmpty()) {
            return "Tous les critères importants ont été respectés dans cet échange.";
        }
        StringBuilder sb = new StringBuilder("Critères non respectés :\n");
        for (Criteria c : notMet) {
            // Utilise le nom brut (raw) du critère, formaté avec des espaces et en minuscules
            String rawName = c.name().toLowerCase().replace('_', ' ');
